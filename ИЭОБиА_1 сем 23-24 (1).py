        ...
        while i != 300:
            ...
            for j in range(0, 4):
                ...
                if str(color) != "FF00B0F0":
                    para_1 = sheet.cell(row=columns_pars + i + j, column=3 + f).value
                    para_2 = sheet.cell(row=columns_pars + i + j, column=4 + f).value
                    if para_1 is not None:
                        day = str(sheet.cell(row=columns_pars + i, column=2).value)
                        para_1 = sheet.cell(row=columns_pars + i + j, column=3 + f).value
                        if para_1 != "None":
                    ...
                    print(name)
                    if name is not None:
                        name = sheet.cell(row=rows, column=3 + f).value
...
def tests():
    book = openpyxl.open("ИЭОБиА_1 сем 23-24 (1).xlsx", read_only=True)  # открытие на чтение
    sheets = book.sheetnames
    for sheet_name in sheets:
        sheet = book[sheet_name]
        print(sheet.cell(row=14, column=3).value)
# db_select()
db_start()
# db_f()
# tests()
