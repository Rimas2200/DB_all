...
def db_start():
    ...
    cursor = db.cursor()
    book = openpyxl.open("расписание_2024/ФЭкологии_2_СЕМ_23-24.xlsx", read_only=True)
    ...
    #     lens = len(name__)
        ...
        while i != 200:
            try:
                name = sheet.cell(row=14, column=3 + f).value
                name_id = name.replace(' ', '')

                color = 0
                try:
                    cell = sheet.cell(row=15 + i, column=2)
                    fill = cell.fill
                    ...
                    if array_week == 7:
                        ...
                        array_week = 1;
                    print(array_week)
                if sheet.cell(row=15 + i, column=2).value is not None:
                    day = str(sheet.cell(row=15 + i, column=2).value)
                else:
                    day = str(sheet.cell(row=14 + i, column=2).value)
                if str(color) != "FF00B0F0":
                    if sheet.cell(row=15+i, column=3+f).value is not None:
                        string = sheet.cell(row=15 + i, column=3+f).value
                        if sheet.cell(row=15 + i, column=4 + f).value is not None:
                    ...
                    if sheet.cell(row=15+i, column=4+f).value is not None:
                        string = sheet.cell(row=15 + i, column=4+f).value
                        try:
                            lessons = string.split("\n")
                            first_two_chars = string[:4]
                            if "1 Н." not in first_two_chars and "2 Н." not in first_two_chars:
