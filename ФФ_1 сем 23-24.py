...
def db_start():
    ...
    db = sqlite3.connect('sql/timetable_db.db')
    cursor = db.cursor()
    book = openpyxl.open("ФФ_1 сем 23-24 (1).xlsx", read_only=True)  # открытие на чтение
    ...
    for sheet_name in sheets:
        ...
        while i != 200:
            ...
            for j in range(0, 4):
                try:
                    cell = sheet.cell(row=columns_pars + i, column=3)
                    fill = cell.fill
                    color = fill.fgColor.rgb if fill.fgColor.rgb is not None else 'No Fill'
                except:
                    pass
                if str(color) != "FF00B0F0":
                    para_1 = sheet.cell(row=columns_pars + i + j, column=4 + f).value
                    para_2 = sheet.cell(row=columns_pars + i + j, column=5 + f).value
                    if para_1 is not None:
                        day = str(sheet.cell(row=columns_pars + i, column=3).value)
                        para_1 = sheet.cell(row=columns_pars + i + j, column=4 + f).value
                        if para_1 != "None":
