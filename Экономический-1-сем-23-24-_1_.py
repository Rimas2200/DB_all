# Управления_1-сем_23-24-_1_.xlsx
# -*- coding: utf-8 -*-
...
def db_start():
    ...
    db = sqlite3.connect('sql/timetable_db.db')
    cursor = db.cursor()
    book = openpyxl.open("Экономический-1-сем-23-24-_1_.xlsx", read_only=True)  # открытие на чтение
    ...
    for sheet_name in sheets:
        ...
        while i != 200:
            subgroup = ''
            try:
                cell = sheet.cell(row=12 + i, column=1)
                fill = cell.fill
            ...
            if str(color) != "FF00B0F0":
                day = sheet.cell(row=12 + i, column=2).value
                try:
                    for j in range(0, 3):
                        para_1 = sheet.cell(row=12 + i + j, column=3 + f).value
                        para = sheet.cell(row=12 + i + j, column=3 + f)
                        if sheet.cell(row=12 + i + j, column=5 + f).value is not None:
                            subgroup = "первая подгруппа"
                        if sheet.cell(row=12 + i + j, column=3 + f).value is not None:
                            lessons.append(sheet.cell(row=12 + i + j, column=3 + f).value)
                        else:
                            lessons.append(None)

                        # print(sheet.cell(row=12 + i + j, column=3 + f), j, color)

                        if sheet.cell(row=11, column=3 + f).value is not None:
                            name_id = sheet.cell(row=11, column=3 + f).value
                            name_id = str(name_id).replace(' ', '-')
                    # i+=3
                    discipline = str(lessons[0])
                    first_two_chars = discipline[:2]
                    first = str(lessons[1])
                    first_two = first[:3]
                    # print(lessons[0], lessons[1], lessons[2])
                    if "1н".lower() not in first_two_chars.lower() and "2н".lower() not in first_two_chars.lower() and "2н".lower() not in first_two.lower():
                        discipline = lessons[0]
                        matches_aud = lessons[2]
                    ...
                    if first_two_chars.lower() == "2н".lower() and lessons[0] is None:
                    ...
                    first_two_chars_ = discipline_[:2]
                    if first_two_chars.lower() == "1н".lower() and first_two_chars_.lower() == "2н".lower():
                        discipline = lessons[0]
                        matches_aud = str(lessons[1])
                        name_with_initials = lessons[0]
                        result = re.search(r"\b([А-Я]\w+\.? [А-Я]\.?[А-Я]?)\b", str(name_with_initials))
                        if result:
                            teacher_name = result.group()
                            name = teacher_name.title()
                    ...
                    first_two_chars = discipline[:2]
                    first_two_chars_ = discipline_[:2]
                    if "1н".lower() in first_two_chars.lower() and "2н".lower() in first_two_chars_.lower():
                        discipline = lessons[0]
                        match = re.search(r"\b([А-Я]-\d+)\b", discipline)
                        if match is not None:
                            matches_aud = match.group(1)
                    ...
                    discipline = str(lessons[2])
                    first_two_chars = discipline[:2]
                    a = lessons[0]
                    b = lessons[1]
                    # print(a, b, discipline, first_two_chars)
                    if "2н" in first_two_chars and a is None and b is None:
                        # print('===============================================')
                        discipline = lessons[2]
                        match = re.search(r"\b([А-Я]-\d+)\b", discipline)
                        if match is not None:
                            ...
                            try:
                                ...
                                pass
                        result = re.search(r"\b([А-Я]\w+\.? [А-Я]\.?[А-Я]?)\b", str(lessons[2]))
                        if result:
                            teacher_name = result.group()
                            name = teacher_name.title()
                            name_with_initials = name[:-2] + ". " + name[-1] + "."
                    ...
                    discipline_ = str(lessons[2])
                    first_two_chars = discipline[:2]
                    if "1н".lower() in first_two_chars.lower() and lessons[2] is None:
                        discipline = lessons[0]
                        pattern = r"\d+\(\d+\)"
                        matches = re.search(pattern, str(lessons[1]))
                        matches_aud = matches.group()
                ...
                try:
                    lessons = []
                    for j in range(0, 3):
                        para_1 = sheet.cell(row=12 + i + j, column=5 + f).value
                        para = sheet.cell(row=12 + i + j, column=5 + f)
                        subgroup = "вторая подгруппа"
                        if sheet.cell(row=12 + i + j, column=5 + f).value is not None:
                            lessons.append(sheet.cell(row=12 + i + j, column=5 + f).value)
                        else:
                            lessons.append(None)
...
def db_selectt():
    ...
    cursor.execute("SELECT DISTINCT group_name FROM timetable")
