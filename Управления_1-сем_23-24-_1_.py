...
def db_start():
    ...
    db = sqlite3.connect('sql/timetable_db.db')
    cursor = db.cursor()
    book = openpyxl.open("Управления_1-сем_23-24-_1_.xlsx", read_only=True)  # открытие на чтение
        ...
        while i != 200:
            ...
            if str(color) != "FF00B0F0":
                ...
                try:
                    ...
                    if ("1Н" or "1н") not in first_two_chars and ("2Н" or "2н") not in first_two_chars and ("2Н" or "2н") not in first_two:
                    ...
                    if first_two_chars == ("2Н" or "2н") and lessons[0] is None:
                    ...
                    if first_two_chars == ("1Н" or "1н") and first_two_chars_ == ("2Н" or "2н"):
                        discipline = lessons[0]
                        match = re.search(r"\b([А-Я]-\d+)\b", discipline)
                    ...
                    if ("1Н" or "1н") in first_two_chars and ("2Н" or "2н") in first_two_chars_:
                        discipline = lessons[0]
                        match = re.search(r"\b([А-Я]-\d+)\b", discipline)
                        if match is not None:
                            matches_aud = match.group(1)
                    ...
                    discipline_ = str(lessons[2])
                    first_two_chars = discipline[:2]
                    if ("1Н" or "1н") in first_two_chars and lessons[2] is None:
                        discipline = lessons[0]
                        match = re.search(r"\b([А-Я]-\d+)\b", discipline)
                        if match is not None:
                            matches_aud = match.group(1)
                ...
                try:
                    lessons = []
                    for j in range(0, 3):
                        para_1 = sheet.cell(row=11 + i + j, column=5 + f).value
                        para = sheet.cell(row=11 + i + j, column=5 + f)
                        subgroup = "вторая подгруппа"
                        if sheet.cell(row=11 + i + j, column=5 + f).value is not None:
                            lessons.append(sheet.cell(row=11 + i + j, column=5 + f).value)
                        else:
                            lessons.append(None)
